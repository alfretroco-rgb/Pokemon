...
Tipo textoTipo(string texto) {
    if (texto == "FUEGO") return FUEGO;
    if (texto == "AGUA") return AGUA;
    if (texto == "PLANTA") return PLANTA;
    if (texto == "ELECTRICO") return ELECTRICO;
    return ROCA;
}
...
void reiniciarVida(Entrenador& entrenador) {
    for (int i = 0; i < TAM_EQUIPO; i++) {
        entrenador.equipo[i]->vida = entrenador.equipo[i]->vidaMaxima;
    }
    entrenador.activo = 0;
}

void batalla(Entrenador& jugador, Entrenador& cpu) {
    reiniciarVida(jugador);
    reiniciarVida(cpu);
...
bool cargarPool(string ruta, EspecieDisponible pool[], int& cantidad) {
    ...
    while (getline(archivo, linea) && cantidad < TAM_POOL) {
        if (linea.empty()) continue;

        string especie, tipo, vida, ataque, defensa;
        ...
        if (especie.empty()) continue;

        pool[cantidad].especie = especie;
        pool[cantidad].tipo = textoTipo(tipo);
        pool[cantidad].vida = stoi(vida);
        pool[cantidad].ataque = stoi(ataque);
        pool[cantidad].defensa = stoi(defensa);
        cantidad++;
    }
...
void mostrarPool(EspecieDisponible pool[], int cantidad) {
    cout << endl << "Pokemon disponibles (fichero):" << endl;
    for (int i = 0; i < cantidad; i++) {
        cout << "  [" << i + 1 << "] " << pool[i].especie;
        cout << " - Tipo: " << tipoTexto(pool[i].tipo);
        cout << " - Vida: " << pool[i].vida;
        cout << " - Ataque: " << pool[i].ataque;
        cout << " - Defensa: " << pool[i].defensa << endl;
    }
}

void reemplazarPokemon(Entrenador& entrenador, int pos, EspecieDisponible datos) {
    delete entrenador.equipo[pos];
    entrenador.equipo[pos] = crearPokemon(datos.especie, datos.especie, datos.tipo, datos.vida, datos.ataque, datos.defensa);
}

void editarEquipoActual(Entrenador& entrenador, EspecieDisponible pool[], int cantidad) {
    mostrarEquipo(entrenador);
    mostrarPool(pool, cantidad);

    cout << "Posicion del equipo a cambiar (1-3): ";
    int pos;
    cin >> pos;

    if (pos < 1 || pos > TAM_EQUIPO) {
        cout << "Posicion invalida." << endl;
        return;
    }

    cout << "Numero del pokemon del listado: ";
    int op;
    cin >> op;
    ...
    reemplazarPokemon(entrenador, pos - 1, pool[op - 1]);
    entrenador.activo = 0;
    cout << "Equipo actualizado." << endl;
}

void menuDatos(Entrenador& jugador, Entrenador& cpu) {
    int op = 0;
    do {
        cout << endl << "===== DATOS =====" << endl;
        cout << "1. Ver equipo del entrenador 1" << endl;
        cout << "2. Ver equipo del entrenador 2" << endl;
        cout << "3. Volver" << endl;
        cout << "Elige una opcion: ";
        cin >> op;

        if (op == 1) mostrarEquipo(jugador);
        else if (op == 2) mostrarEquipo(cpu);
        else if (op != 3) cout << "Opcion invalida." << endl;
    } while (op != 3);
}

void menuFicheros(Entrenador& jugador, Entrenador& cpu, EspecieDisponible pool[], int cantidad) {
    int op = 0;
    do {
        cout << endl << "===== GESTIONAR FICHEROS =====" << endl;
        cout << "1. Ver listado de Pokemon disponibles (fichero)" << endl;
        cout << "2. Editar equipo actual (seleccionar del listado)" << endl;
        cout << "3. Volver" << endl;
        cout << "Elige una opcion: ";
        cin >> op;
        ...
        if (op == 1) {
            ...
            cout << "1. Editar Jugador" << endl;
            cout << "2. Editar CPU" << endl;
            cout << "Elige entrenador: ";
            int ent;
            cin >> ent;

            if (ent == 1) editarEquipoActual(jugador, pool, cantidad);
            else if (ent == 2) editarEquipoActual(cpu, pool, cantidad);
            else cout << "Entrenador invalido." << endl;
        } else if (op != 3) {
...
void menuPrincipal() {
    ...
    cout << "1. Mochila" << endl;
    cout << "2. Datos" << endl;
    cout << "3. Iniciar batalla" << endl;
    cout << "4. Gestionar ficheros" << endl;
    cout << "5. Salir" << endl;
    cout << "Elige una opcion: ";
}
...
int main() {
    ...
    EspecieDisponible pool[TAM_POOL];
    int cantidadPool = 0;
    if (!cargarPool("pokemon_disponibles.txt", pool, cantidadPool)) {
        cout << "Aviso: no se pudo cargar pokemon_disponibles.txt" << endl;
    }
    ...
    do {
        ...
        if (opcion == 1) {
            ...
            menuDatos(jugador, cpu);
        } else if (opcion == 3) {
            batalla(jugador, cpu);
        } else if (opcion == 4) {
            if (cantidadPool > 0) menuFicheros(jugador, cpu, pool, cantidadPool);
            else cout << "No hay datos de fichero para gestionar." << endl;
        } else if (opcion != 5) {
            cout << "Opcion invalida." << endl;
        }
    } while (opcion != 5);
